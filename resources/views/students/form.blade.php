<input name="student_id" value="{{ old('student_id', $student?->student_id) }}" class="form-control" placeholder="Student ID" required>
...
<select name="school_class_id" class="form-control" required><option value="">Class</option>@foreach($classes as $class)<option value="{{ $class->id }}" @selected(old('school_class_id',$student?->school_class_id)==$class->id)>{{ $class->class_name }}</option>@endforeach</select>
