    ...
    <aside class="fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-slate-200 bg-white transition-all duration-200 lg:z-30"
        ...
        <nav class="flex-1 space-y-1 px-3 pb-4">
            ...
            @if(auth()->user()?->role === 'admin')
                <a href="{{ route('course-types.index') }}" class="sidebar-link {{ request()->routeIs('course-types.*') ? 'sidebar-link-active' : '' }}"><svg class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v12m6-6H6"/></svg><span x-show="!sidebarCollapsed">Course types</span></a>
