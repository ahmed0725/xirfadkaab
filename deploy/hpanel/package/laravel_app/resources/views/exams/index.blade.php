        ...
        <form method="GET" class="card grid gap-3 md:grid-cols-5">
            <select name="school_class_id" class="form-control">
                ...
                @foreach($classes as $class)
                    <option value="{{ $class->id }}" @selected(($filters['school_class_id'] ?? '') == $class->id)>{{ $class->display_name }}</option>
                @endforeach
            </select>
            <select name="subject_id" class="form-control">
                ...
                @foreach($subjects as $subject)
                    <option value="{{ $subject->id }}" @selected(($filters['subject_id'] ?? '') == $subject->id)>
                        {{ $subject->subject_name }} ({{ $subject->schoolClass?->display_name }})
                    </option>
                @endforeach
            </select>
                ...
                <tbody>
                    @forelse($exams as $exam)
                        <tr class="border-t border-slate-100 hover:bg-slate-50">
                            ...
                            <td class="p-3">{{ $exam->schoolClass?->display_name }}</td>
