...
<select name="school_class_id" class="border rounded p-2" required>@foreach($classes as $class)<option value="{{ $class->id }}" @selected($class->id==$subject->school_class_id)>{{ $class->class_name }}</option>@endforeach</select>
