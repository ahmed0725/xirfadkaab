    ...
    <div class="space-y-4">
        ...
        <div class="table-shell overflow-x-auto">
            <table class="w-full text-sm">
                <thead><tr class="bg-slate-50 text-left text-slate-600"><th class="p-3">ID</th><th class="p-3">Name</th><th class="p-3">Class</th><th class="p-3">Status</th><th class="p-3">Actions</th></tr></thead>
                <tbody>
                @foreach($students as $student)
                    <tr class="border-t border-slate-100 hover:bg-slate-50"><td class="p-3">{{ $student->student_id }}</td><td class="p-3">{{ $student->name }}</td><td class="p-3">{{ $student->schoolClass->class_name }}</td><td class="p-3"><span class="rounded-full px-2 py-1 text-xs {{ $student->status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">{{ $student->status }}</span></td>
